                ...
                >
                    <div>
                        ...
                        <Paragraph color='text.muted'>
                            {lang === "uz"
                                ? `Jami ${data?.pagination?.total} ta topildi`
                                : `Всего найдено ${data?.pagination?.total}`}
                        </Paragraph>
                    </div>
                    ...
                    <Grid item lg={9} xs={12}>
                        {data?.products?.length !== 0 ? (
                            ...
                            >
                                <Typography>
                                    {lang === "uz"
                                        ? "Kechirasiz, hozirda ushbu kategoriyada hech qanday mahsulot yoʻq."
                                        : "К сожалению, в данной категории временно нет товаров"}
                                </Typography>
    ...
    if (params.id !== "products") {
        const res = await fetch(
            `https://ecommerce.main-gate.appx.uz/dev/v1/category/${
                params.id
            }?per_page=15${query.page ? `&page=${query.page}` : ""}${
                query.brand_ids ? `&brand_ids=${query.brand_ids}` : ""
            }${query.attr ? `&attr=${query.attr}` : ""}${
                query.sort_by
                    ? `&sort=${query.sort_by},${query.order_type}`
                    : ""
            }`
        );
        ...
        const res = await fetch(
            `https://ecommerce.main-gate.appx.uz/dev/v1/product/list?per_page=15&page=${
                query.page
            }${query.brand ? `&brand_ids=${query.brand}` : ""}${
                query.sort_by
                    ? `&sort=${query.sort_by},${query.order_type}`
                    : ""
            }${query.views ? `&sort=${query.views}` : ""}${
                query.news ? `&sort=${query.news}` : ""
            }${query.type ? `&type=${query.type}` : ""}${
                query.search ? `&type=default&search=${query.search}` : ""
            }`
        );
        const data = await res.json();
        return {
