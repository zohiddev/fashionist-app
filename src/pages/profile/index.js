...
const Profile = () => {
	...
	const infoList = [
		...
		{
			title: state?.user?.orders?.filter(item => item?.status === 1).length,
			subtitle: lang === 'uz' ? 'Jarayonda' : 'В Процессе',
		},
		{
			title: state?.user?.orders?.filter(item => item?.status === 2).length,
			subtitle: lang === 'uz' ? 'Yo`lda' : 'В дороге',
		},
		{
			title: state?.user?.orders?.filter(item => item?.status === 3).length,
	...
	return (
		<Container mt='50px' mb='50px'>
			<DashboardPageHeader
				...
				title={lang === 'uz' ? "Mening profilim" : 'Мой профиль'}
				button={
					<Link href="/profile/edit">
						<Button color="primary" bg="primary.light" px="2rem">
							{lang === 'uz' ? 'Profilni o`zgartirish' : 'Изменить профиль'}
						</Button>
					</Link>
				}
				exitBtn={true}
					...
					<Grid item lg={6} md={6} sm={12} xs={12}>
						...
						>
							...
							<Box ml="12px" flex="1 1 0">
								...
								>
									<div>
										<H5 my="0px">{state?.user?.user?.full_name ? state?.user?.user?.full_name : lang === 'uz' ? 'Ism kiritilmagan' : 'Имя не введено'}</H5>
										<H5 my="0px">{state?.user?.user?.phone}</H5>
									</div>
								</FlexBox>
							</Box>
						</FlexBox>
...

			{
				...
				<>
					<DashboardPageHeader
						title={lang === 'uz' ? "Mening buyurtmalarim" : 'Мои заказы'}
						iconName="bag_filled"
						mt="50px"
					/>
					...
					<Hidden down={769}>
						<TableRow padding="0px 18px" boxShadow="none" bg="none">
							...
							<H5 color="text.muted" my="0px" mx="6px" textAlign="left">
								{lang === 'uz' ? 'Sotib olingan sana' : 'Дата покупки'}
							</H5>
							<H5 color="text.muted" my="0px" mx="6px" textAlign="left">
								{lang === 'uz' ? 'Jami' : 'Всего'}
							</H5>
