...
const ProductDetails = ({ data }) => {
  ...
  useEffect(() => {
    ...
    dispatch(
      setHeaderContent(makeKeywords(data.product.name_uz, data.product.name_ru))
    );
  }, []);
      ...
      <Container mb="70px" mt="50px">
        ...
        >
          <H5
            ...
            p="4px 10px"
            color={
              selectedOption === "attributes" ? "primary.main" : "text.muted"
            }
        ...
        <Box mb="50px">
          ...
          {selectedOption === "attributes" && (
            <ProductDescription attributes={data?.product?.attributes} />
          )}
        </Box>

        {data?.recommendedProducts?.length === 0 ? null : (
          <RelatedProducts recommendedProducts={data?.recommendedProducts} />
        )}
      </Container>
    </div>
  );
